    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div class="card bg-info/10 border border-info/20 shadow-sm">
            <div class="card-body p-5 flex flex-row items-center gap-4">
                ...
                <div>
                    <div class="text-2xl font-bold text-info">{{ $siswaSedangUjian }}</div>
            ...
            <div class="card-body p-0">
                ...
                @if($monitoringUjian->isEmpty())
                    <div class="flex flex-col items-center justify-center py-12 text-center">
                        <div class="w-12 h-12 bg-base-200 rounded-full flex items-center justify-center mb-3 text-base-content/30">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                stroke-width="1.5" stroke="currentColor" class="size-6">
                                <path stroke-linecap="round" stroke-linejoin="round"
                        ...
                        <tbody class="divide-y divide-base-200">
                            ...
                            <tr class="hover:bg-base-200/20 align-middle">
                                <td>
                                    ...
                                    <div class="text-xs text-primary font-medium">{{ $paket->mataPelajaran->nama ?? '-' }}</div>
                                </td>
                                <td class="text-center">
                                    @if($paket->sedang_mengerjakan > 0)
                                        <span class="badge badge-info badge-sm font-bold">{{ $paket->sedang_mengerjakan }}</span>
                                    @else
                                        <span class="text-base-content/25 text-xs">—</span>
                                    @endif
                                </td>
                                <td class="text-center text-sm font-bold text-success">{{ $paket->sudah_submit }}</td>
                                <td class="text-center text-sm text-base-content/60">{{ $paket->total_peserta }}</td>
                                <td class="min-w-28">
                                    <div class="flex items-center gap-2">
                                        <progress class="progress progress-primary w-full h-1.5" value="{{ $pct }}" max="100"></progress>
                                        <span class="text-[10px] font-bold text-base-content/50 w-7 shrink-0">{{ $pct }}%</span>
                                    </div>
                                </td>
                            </tr>
                            @endforeach
        ...
        <div class="xl:col-span-2 card bg-base-100 border border-base-200 shadow-sm">
            <div class="card-body p-0">
                <div class="flex items-center justify-between px-5 py-4 border-b border-base-200">
                    ...
                    <a href="{{ route('admin.rekap.index') }}" class="btn btn-xs btn-ghost text-primary font-semibold">
                        Lihat semua →
                    </a>
                </div>
                ...
                <div class="overflow-x-auto">
                    <table class="table table-sm w-full">
                        ...
                        <tbody class="divide-y divide-base-200">
                            ...
                            <tr class="hover:bg-base-200/20 align-middle">
                                <td>
                                    <div class="text-sm font-bold text-base-content truncate max-w-48">{{ $paket->nama }}</div>
                                    <div class="text-xs text-primary font-medium">{{ $paket->mataPelajaran->nama ?? '-' }}</div>
                                </td>
                                <td class="text-center text-sm text-base-content/70">{{ $paket->jumlah_peserta }}</td>
                                <td class="text-center">
                                    <span class="text-sm font-bold {{ $rata >= 75 ? 'text-success' : 'text-error' }}">
                                        {{ $rata }}
                                    </span>
                                </td>
                                <td class="text-center text-sm font-semibold text-success">{{ $paket->nilai_tertinggi ?? '—' }}</td>
                                <td class="text-center text-sm font-semibold text-error">{{ $paket->nilai_terendah ?? '—' }}</td>
                                <td class="text-center">
